...
// ============ IPC Adapter Tests ============
const ipcAdapterTests = new TestRunner('IPC Adapter Tests');

ipcAdapterTests.test('IPCMessage serialization and deserialization works correctly', async () => {
  const { IPCMessage, IPCMessageType } = await import('./src/adapters/desktop/ipc-adapter.js');
  
  const testPayload = { channel: 'test_channel', data: { value: 42, name: 'Test' } };
  const message = new IPCMessage(IPCMessageType.REQUEST, testPayload, {
    metadata: { extra: 'info' },
    source: 'renderer',
    target: 'main'
  });
  ...
  if (jsonData.type !== IPCMessageType.REQUEST || jsonData.payload.data.value !== 42) {
    throw new Error(`Message serialization failed: ${JSON.stringify(jsonData)}`);
  }
  
  const reconstructed = IPCMessage.fromJSON(jsonData);
  if (reconstructed.type !== IPCMessageType.REQUEST || reconstructed.payload.data.name !== 'Test') {
    throw new Error(`Message deserialization failed: ${JSON.stringify(reconstructed)}`);
  }
});

ipcAdapterTests.test('MessageQueue functionality works', async () => {
  const { MessageQueue } = await import('./src/adapters/desktop/ipc-adapter.js');
  ...
  if (queue.size() !== 3) {
    throw new Error('Queue size should be 3');
  }
  
  queue.enqueue({ id: 4 }); // Should evict oldest
  const first = queue.dequeue();
  if (first.id !== 2) {
    throw new Error(`Expected id 2, got ${first.id}`);
  }
...
ipcAdapterTests.test('MainProcessIPCAdapter validation works', async () => {
  const { MainProcessIPCAdapter, IPCMessage, IPCMessageType } = await import('./src/adapters/desktop/ipc-adapter.js');
  ...
  const adapter = new MainProcessIPCAdapter(mockIpcMain, mockEventBus, { validateMessages: true });
  
  // Test valid message
  const validMessage = new IPCMessage(IPCMessageType.REQUEST, { test: 'data' });
  const validation1 = adapter.validateMessage(validMessage);
  if (!validation1.valid) {
    throw new Error(`Valid message should pass validation: ${validation1.error}`);
  }
  
  // Test invalid message
  const validation2 = adapter.validateMessage({ type: null });
  if (validation2.valid) {
    throw new Error('Invalid message should fail validation');
  }
});

ipcAdapterTests.test('IPCAdapterBase public methods are available', async () => {
  const { IPCAdapterBase, IPCMessage, IPCMessageType } = await import('./src/adapters/desktop/ipc-adapter.js');
  
  // Create a test subclass
  class TestIPCAdapter extends IPCAdapterBase {
    async send(message) { return message; }
    async connect() { this.isConnected = true; }
  }
  
  const adapter = new TestIPCAdapter();
  const reqId = adapter.generateRequestId();
  if (!reqId || typeof reqId !== 'string') {
    throw new Error('generateRequestId should return a string');
  }
  
  const testMessage = new IPCMessage(IPCMessageType.REQUEST, { test: 'data' });
  const validation = adapter.validateMessage(testMessage);
  if (!validation.valid) {
    throw new Error(`Valid message should pass validation: ${validation.error}`);
  }
  
  const response = adapter.createResponse(testMessage, { result: 'ok' });
  if (response.type !== IPCMessageType.RESPONSE) {
    throw new Error('createResponse should return RESPONSE type message');
  }
  
  const error = adapter.createError(testMessage, new Error('test error'));
  if (error.type !== IPCMessageType.ERROR) {
    throw new Error('createError should return ERROR type message');
  }
  
  const event = adapter.createEvent('test_event', { data: 'payload' });
  if (event.type !== IPCMessageType.EVENT) {
    throw new Error('createEvent should return EVENT type message');
  }
});

ipcAdapterTests.test('IPC adapter stats reporting works', async () => {
  const { MainProcessIPCAdapter } = await import('./src/adapters/desktop/ipc-adapter.js');
  const { EventEmitter } = await import('events');
  
  const mockIpcMain = new EventEmitter();
  const mockEventBus = new EventEmitter();
  
  const adapter = new MainProcessIPCAdapter(mockIpcMain, mockEventBus);
  const stats = adapter.getStats();
  
  if (typeof stats.isConnected !== 'boolean' ||
      typeof stats.pendingRequests !== 'number' ||
      typeof stats.queueSize !== 'number') {
    throw new Error(`Stats object has unexpected format: ${JSON.stringify(stats)}`);
  }
});
