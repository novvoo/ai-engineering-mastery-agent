...
- 默认可使用本地 fallback embedder；运行 `/doc init` 时如果 ONNX 模型缺失且自动下载开启，会先尝试自动下载模型，再检查 `onnxruntime-node` / tokenizer / ONNX session 是否可用。
- ONNX 模式需要依赖 `onnxruntime-node` 和 `@huggingface/tokenizers`，并且需要同一模型 repo 下的 `onnx/model.onnx`、`tokenizer.json` 和 `tokenizer_config.json`。默认使用 `onnx-community/gte-multilingual-base`，对中文和跨语言检索更友好，缓存路径会自动放在一起。
- 默认使用 CLS pooling；如需切换为 mean pooling，可设置 `EMBEDDING_POOLING=mean`。
- 如果看到 `ONNX Runtime initialization failed, using fallback`，先运行 `/doc init`。它会告诉你是没有安装 ONNX runtime、tokenizer 加载失败、模型或 tokenizer 文件不存在、下载失败，还是已经成功使用 fallback。
- 默认模型缓存目录是 `~/.cache/ai-engineering-mastery-agent/models/...`，可用 `AEMA_MODEL_CACHE_DIR` 覆盖。
- 下载候选包含 HuggingFace 官方 `https://huggingface.co`、`HF_ENDPOINT`、`EMBEDDING_MODEL_MIRRORS` 和默认 `https://hf-mirror.com`；实际下载前会先用 HEAD 轻量探测候选源，并选择当前可用且响应更快的源，避免对多个镜像重复下载同一个大模型。
- 默认 ONNX 模型约 1.26 GiB，单个下载 URL 默认超时为 10 分钟；网络慢时可调大 `EMBEDDING_MODEL_DOWNLOAD_TIMEOUT_MS`。如需更小模型文件，可用 `EMBEDDING_MODEL_FILE=onnx/model_quantized.onnx` 覆盖。
...
| `EMBEDDING_TOKENIZER_PATH` | 模型缓存同级 `tokenizer.json` | 手动指定 embedding tokenizer JSON |
| `EMBEDDING_TOKENIZER_CONFIG_PATH` | 模型缓存同级 `tokenizer_config.json` | 手动指定 embedding tokenizer config |
| `EMBEDDING_MODEL_AUTO_DOWNLOAD` | `true` | 模型缺失时是否自动下载 |
| `EMBEDDING_MODEL_REPO` | `onnx-community/gte-multilingual-base` | HuggingFace embedding 模型 repo |
| `EMBEDDING_MODEL_REVISION` | `main` | HuggingFace 模型 revision |
| `EMBEDDING_MODEL_FILE` | `onnx/model.onnx` | repo 内 ONNX 文件路径 |
| `EMBEDDING_POOLING` | `cls` | ONNX 输出池化方式，可选 `cls` 或 `mean` |
...
| `EMBEDDING_MODEL_PROBE_TIMEOUT_MS` | `10000` | 单个模型候选源轻量探测超时时间 |
| `EMBEDDING_MODEL_DOWNLOAD_TIMEOUT_MS` | `600000` | 单个模型下载 URL 超时时间 |
