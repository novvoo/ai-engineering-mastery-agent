...
- ONNX 模式需要依赖 `onnxruntime-node` 和 `@huggingface/tokenizers`，并且需要同一模型 repo 下的 `onnx/model.onnx`、`tokenizer.json` 和 `tokenizer_config.json`。默认缓存路径会自动放在一起。
...
- 默认 ONNX 模型约 568 MiB，单个下载 URL 默认超时为 10 分钟；网络慢时可调大 `EMBEDDING_MODEL_DOWNLOAD_TIMEOUT_MS`。
