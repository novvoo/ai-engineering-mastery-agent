...
import { StateGraph, ContextProjectionEngine } from './state-graph-core.js';
import { CompleteIndex, SymbolIndexer, DependencyAnalyzer } from './content-addressable-store.js';
...
export class ContextProjectionGenerator {
   ...
   * 为编辑任务生成投影
   */
  projectForEditing(filePath, lineNumber, options) {
    const opts = {
   ...
   * 为理解任务生成投影
   */
  projectForUnderstanding(query, relevantFiles, options) {
    const opts = {
   ...
   * 生成智能投影：根据任务自动选择内容
   */
  projectSmart(taskType, focus, options) {
...
`;
   ...
   * 生成额外的上下文
   */
  generateAdditionalContext(filePath, lineNumber, relevantSymbol) {
    const parts = [];
...
export class HistoryProjection {
   ...
   * 生成差异视图
   */
  generateDiffView(fromCommitId, toCommitId) {
