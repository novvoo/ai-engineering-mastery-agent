...
export function createPtyTools() {
  return [
    {
      name: 'pty_start',
      description: 'Start a command in a persistent pseudo-terminal. Use this instead of shell for interactive commands, REPLs, prompts, watch/dev servers, TUIs, or long-running processes that need incremental output or stdin.',
      category: ToolCategory.SYSTEM,
      params: {
        command: { type: 'string', description: 'Command to start inside the PTY' },
        cwd: { type: 'string', description: 'Optional working directory relative to the agent working directory' },
        cols: { type: 'number', description: 'Terminal columns (default 120)' },
        rows: { type: 'number', description: 'Terminal rows (default 30)' },
        wait_ms: { type: 'number', description: 'Milliseconds to wait before returning initial output (default 300)' },
        max_chars: { type: 'number', description: 'Maximum output characters to return (default 6000)' },
      },
      required: ['command'],
    ...
    {
      name: 'pty_write',
      description: 'Write input to an existing PTY session. Use for answering prompts, sending REPL commands, pressing Enter, or interrupting with control characters such as \\u0003.',
      category: ToolCategory.SYSTEM,
      params: {
        ...
        wait_ms: { type: 'number', description: 'Milliseconds to wait after writing before returning output (default 300)' },
        max_chars: { type: 'number', description: 'Maximum output characters to return (default 6000)' },
      },
    ...
    {
      name: 'pty_read',
      description: 'Read buffered output from an existing PTY session without writing input. Use to check progress of dev servers, watch commands, tests, or interactive programs.',
      category: ToolCategory.SYSTEM,
      params: {
        session_id: { type: 'string', description: 'PTY session id returned by pty_start' },
        cursor: { type: 'number', description: 'Optional output_cursor from a previous PTY response to read only new output' },
    ...
    {
      name: 'pty_stop',
      description: 'Stop an existing PTY session. Use when a long-running command or dev server is no longer needed.',
      category: ToolCategory.SYSTEM,
      params: {
        session_id: { type: 'string', description: 'PTY session id returned by pty_start' },
        signal: { type: 'string', description: 'Signal to send on POSIX systems (default SIGTERM)' },
      },
      required: ['session_id'],
