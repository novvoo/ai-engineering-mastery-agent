  ...
  return [
    {
      ...
      description: '查询工作区状态：已知存在的文件/目录、不存在的路径、关键事实。用于避免重复检查已知信息。',
      category: 'System',
      parameters: {
        type: 'object',
        properties: {
      ...
      handler: async (args, context) => {
        ...
        switch (action) {
          case 'check_path': {
            ...
            const reason = exists === 'not_found' 
              ? workspaceState.getPathNotFoundReason(path)
              : null;
          ...
          case 'get_summary': {
            ...
            const workspaceDescription = context?.observationSummarizer?.generateWorkspaceDescription?.() 
              || '无法生成描述';
          ...
          case 'get_not_found': {
            ...
            return {
              ...
              paths: notFoundPaths,
              suggestion: notFoundPaths.length > 0
